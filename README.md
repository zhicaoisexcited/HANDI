...
1. Download our [video dataset](https://prism.eecs.umich.edu/zhicao/IVG/video_data/) which are preprocessed subsets of the EPIC-KITCHENS/EGO4D. Also, downlaod the corresponding [prompt file](https://prism.eecs.umich.edu/zhicao/IVG/prompt_file/). Put them under `downloads/dataset/`.
...
4. Download our [region of motion mask](https://prism.eecs.umich.edu/zhicao/IVG/mask/) of the video dataset and put it under `downloads/masks/`. Then change `mask_path` under `VideoJsonDataset` class in `utils/dataset.py`.
