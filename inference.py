...
from models.pipeline import LatentToVideoPipeline, TextToVideoSDPipeline
...
from hand import hand_pose_loss
...
def finetune_unet(accelerator, batch, use_offset_noise,
    ...
    hand_loss = hand_pose_loss(predict_x0, latents, vae)
    print("hand_loss:", hand_loss)
...
def batch_eval(unet, text_encoder, vae, vae_processor, pretrained_model_path, 
    validation_data, output_dir, preview, global_step=0, iters=1):
    device = vae.device
    dtype = vae.dtype
    unet.eval()
    text_encoder.eval()
...
def evaluate_folder(
    ...
    # Loop through each image in the folder and evaluate
    for item in prompts:
        source_image = os.path.join(image_folder, item['source'])
        prompt = item['prompt']
        image_number = os.path.basename(source_image).split('.')[0].split('/')[-1]
        mask_image = os.path.join(mask_folder, f"{image_number}.png")
        
        out_file_dir = os.path.join(output_folder, image_number)
        if os.path.exists(out_file_dir) and len(os.listdir(out_file_dir)) > 0:
            print(f"Skipping {source_image} as it has already been processed.")
            continue
        
        validation_data.prompt_image = source_image
        validation_data.prompt = prompt
        validation_data.stregth = 5
        validation_data.mask = mask_image
        
        # Perform evaluation for each image
        batch_eval(unet, text_encoder, vae, vae_processor, pretrained_model_path, 
                   validation_data, output_folder, True)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=str, default="./configs/my_config.yaml")
    parser.add_argument("--image_folder", type=str, required=True, help="Path to folder containing source images.")
    parser.add_argument("--prompt_file", type=str, required=True, help="Path to the JSON file containing prompts.")
    parser.add_argument("--mask_folder", type=str, required=True, help="Path to folder containing mask images.")
    parser.add_argument("--output_folder", type=str, default="./output", help="Path to folder to save outputs.")
    ...
    if args.eval:
        evaluate_folder(
            ...
            validation_data = args_dict.validation_data,
            image_folder=args.image_folder,
            prompt_file=args.prompt_file,
            mask_folder=args.mask_folder,
            output_folder=args.output_folder,
            enable_xformers_memory_efficient_attention=args_dict.enable_xformers_memory_efficient_attention,
            enable_torch_2_attn=args_dict.enable_torch_2_attn,
            seed=args_dict.seed,
            motion_strength=1
        )
    else:
