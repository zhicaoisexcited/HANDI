...
# def save_mask_as_image(mask, save_dir, file_name_prefix, mask_index):
#     # 确保保存目录存在
#     os.makedirs(save_dir, exist_ok=True)

#     # 将 mask 转换为 numpy array
#     mask_np = mask.cpu().detach().numpy()
    
#     # 对 batch 中的第一个 mask 保存
#     print("mask_np.shape", mask_np.shape)
#     mask_img = mask_np[0, 0, :, :]
...
#     # 使用文件名前缀和索引生成唯一的文件名
#     img_path = os.path.join(save_dir, f"{file_name_prefix}_{mask_index}.png")
#     img.save(img_path)
#     print(f"Saved mask image to {img_path}")
...
class LoopingSampler(Sampler):
    def __init__(self, data_source, batch_size, shuffle=True):
        ...
        self.dataset_length = len(self.data_source)
        self.num_batches = math.ceil(self.dataset_length / self.batch_size)
    ...
    def __iter__(self):
        ...
        full_cycles = self.num_samples // self.dataset_length
        remainder = self.num_samples % self.dataset_length

        all_indices = indices * full_cycles + indices[:remainder]
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    # extra_vae_params = extra_vae_params if extra_vae_params is not None else {}
    ...
    optim_params = [
        ...
        # param_optim(vae, True, extra_params=extra_vae_params)
    ]
    ...
    sampler = LoopingSampler(train_dataset, batch_size=train_batch_size, shuffle=shuffle)
...
def finetune_unet(accelerator, batch, use_offset_noise,
    ...
    # pixel_values: torch.Size([48, 16, 3, 256, 256])
    ...
    # latents: torch.Size([48, 4, 16, 32, 32])
    video_length = latents.shape[2]
    condition_latent = latents[:,:, 0:1].detach().clone()

    #mask_shape: (2,256,156)
    mask = batch["mask"]
    ...
    # mask1: torch.Size([40, 256, 256])
    mask1 = batch["mask1"]
    mask1 = mask1.div(255).to(dtype)
    mask1[mask1<0.5] = 0
    mask1[mask1>=0.5] = 1
    
    # mask1 = mask1.cpu().numpy()
    # output_txt_path = '/home/zhicao/kitchen/1.txt'
    ...
    # print(f"Pixel values have been saved to {output_txt_path}")
    ...
    # model_pred: torch.Size([48, 4, 16, 32, 32])
    # target: torch.Size([48, 4, 16, 32, 32])
